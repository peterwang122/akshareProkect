...
def test_next_us_business_day_uses_complete_federal_calendar(trade_date, expected):
    assert index._next_us_business_day(trade_date) == expected
...
def test_credit_backfill_fills_available_at_for_legacy_database_rows(monkeypatch):
    monkeypatch.setattr(
        index,
        "fetch_fred_series_csv",
...
def test_align_by_available_at_uses_explicit_cn_holiday_calendar():
    points = [
        {
            "source_date": "2026-02-13",
            "value": 4.5,
            "available_at": "2026-02-14T05:00:00+08:00",
        }
    ]
    aligned = quant_index.align_metric_points_to_cn_dates_by_available_at(
        points,
    ...
    assert aligned["2026-02-13"]["source_date"] == "2026-02-13"


def test_align_by_available_at_uses_explicit_cn_national_day_calendar():
    points = [
        {
            "source_date": "2026-09-30",
            "value": 4.5,
            "available_at": "2026-10-01T05:00:00+08:00",
        }
    ]
    aligned = quant_index.align_metric_points_to_cn_dates_by_available_at(
        points,
    ...
    assert aligned["2026-09-30"]["source_date"] == "2026-09-30"


def test_align_by_available_at_does_not_guess_latest_cn_trade_date():
    aligned = quant_index.align_metric_points_to_cn_dates_by_available_at(
        [
            {
                "source_date": "2026-02-13",
            ...
            [
                {
                    ...
                    "available_at": f"{trade_date}T06:00:00+08:00",
                    "data_source": "test",
                },
                {
                    "asset_code": "IXN_NAV",
...
class _MigrationCursor:
    ...
    async def execute(self, query, *_args):
        ...
        self._rows = [] if "information_schema.columns" in normalized else self._rows
...
def test_existing_us_macro_tables_receive_idempotent_column_migrations():
    cursor = _MigrationCursor()
    connection = _MigrationConnection(cursor)
    db = DbTools.__new__(DbTools)
    db.pool = _MigrationPool(connection)
    db._index_us_macro_auxiliary_tables_ready = False

    asyncio.run(db.ensure_index_us_macro_auxiliary_tables())
    alter_statements = [
        statement for statement in cursor.executed if statement.startswith("ALTER TABLE")
    ]
    assert any("yield_real_10y" in statement for statement in alter_statements)
    assert sum("available_at" in statement for statement in alter_statements) == 2
    first_statement_count = len(cursor.executed)

    asyncio.run(db.ensure_index_us_macro_auxiliary_tables())
    assert len(cursor.executed) == first_statement_count


def test_available_at_is_normalized_for_mysql_datetime():
    db = DbTools.__new__(DbTools)
    row = db._sanitize_index_us_treasury_yield_row(
        {
            "trade_date": "2026-08-20",
            "available_at": "2026-08-21T05:00:00+08:00",
        }
    )
    assert row["available_at"] == datetime(2026, 8, 21, 5, 0)
