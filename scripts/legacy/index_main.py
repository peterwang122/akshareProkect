...
def save_progress_batch(progress_lines):
    if not progress_lines:
        return
    with open(INDEX_PROGRESS_LOG, 'a', encoding='utf-8') as file:
...
def load_progress():
    if not os.path.exists(INDEX_PROGRESS_LOG):
        return set()
    with open(INDEX_PROGRESS_LOG, 'r', encoding='utf-8') as file:
        return {line.strip() for line in file if line.strip()}


def log_error(index_code, trade_date, error_message):
    with open(INDEX_ERROR_LOG, 'a', encoding='utf-8') as file:
        file.write(f'{index_code},{trade_date},{error_message}\n')
