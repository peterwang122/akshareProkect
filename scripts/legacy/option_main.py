...
MAX_CONCURRENCY = 6
OPTION_PROGRESS_LOG = 'option_progress.log'
OPTION_ERROR_LOG = 'option_error.log'
...
def load_progress():
    if not os.path.exists(OPTION_PROGRESS_LOG):
        return set()
    with open(OPTION_PROGRESS_LOG, 'r', encoding='utf-8') as file:
        return {line.strip() for line in file if line.strip()}


def save_progress(option_symbol):
    with open(OPTION_PROGRESS_LOG, 'a', encoding='utf-8') as file:
        file.write(f'{option_symbol}\n')


def log_error(context, error_message):
    with open(OPTION_ERROR_LOG, 'a', encoding='utf-8') as file:
        file.write(f'{context},{error_message}\n')
